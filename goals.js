...
function loadGoals() {
 let text = "";
 if (localStorage.getItem("goalsdata") != null) {
  goalsList = JSON.parse(localStorage.getItem("goalsdata"));
 }
 ...
 text += "<div class=\"container\"><div class=\"row\"><div class=\"col\"><button type=\"button\" class=\"btn btn-primary goals\" data-toggle=\"modal\" data-target=\"#add-menu\">+ Add New Goals</button></div>";

 // add modal for "add new data"
 text += "<div style=\"margin: auto;\" class=\"modal fade\" id=\"add-menu\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"addgoals\" aria-hidden=\"true\"> <div class=\"modal-dialog\" role=\"document\"><div class=\"modal-content\"><div class=\"modal-header\"><h5 class=\"modal-title\" id=\"addgoals\">Add New Goals</h5><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button></div><div class=\"modal-body\"><form method=\"get\"><div class=\"form-group\"><label for=\"new-goals\" class=\"desc-label\" style=\"margin-bottom: 5px; text-align: left\">Enter Goals Description</label><input type=\"text\" autocomplete=\"off\" class=\"form-control\" id=\"new-goals\" aria-describedby=\"emailHelp\" placeholder=\"Example: Test\" required></div>";

 // If user choose 'Juz'
 text += "<div class=\"form-check col-sm-1\" style=\"margin-top: 5px;\"><label class=\"form-check-label\" for=\"flexCheckDefault\"\"><input class=\"form-check-input\" type=\"checkbox\" value=\"\" id=\"juz-checkbox\" onclick=\'displayJuzAddData()\'>Juz</label></div><div class=\"form-inline w-50\" id=\"juz-choose-section\" style=\"display: none; text-align: center; margin: auto;\"><label for=\"exampleInputPassword1\" class=\"row align-items-center\" style=\"margin-right: 5px;\">From</label><input type=\"number\" min=\"1\" max=\"30\" class=\"form-control\" id=\"minJuz\" placeholder=\"\"><label for=\"exampleInputPassword1\" style=\"margin: 0 5px 0 5px;\" class=\"row align-items-center\">To</label><input type=\"number\" min=\"1\" max=\"30\" class=\"form-control\" id=\"maxJuz\" placeholder=\"\"></div>";

 // If user choose 'Surah' - to be determined
 /* text += "<div class=\"form-check col-sm-1\" style=\"margin-top: 5px;\"><label class=\"form-check-label\" for=\"flexCheckDefault\"\"><input class=\"form-check-input\" type=\"checkbox\" value=\"\" id=\"surah-checkbox\" onclick=\'displaySurahAddData()\'>Surah</label></div><div class=\"form-inline w-50\" id=\"surah-choose-section\" style=\"display: none; text-align: left; margin: auto;\"><label for=\"exampleInputPassword1\" class=\"row align-items-center\" style=\"margin-right: 5px;\">From</label><input type=\"number\" min=\"1\" max=\"30\" class=\"form-control\" id=\"exampleInputPassword1\" placeholder=\"\"><label for=\"exampleInputPassword1\" style=\"margin: 0 5px 0 5px;\" class=\"row align-items-center\">To</label><input type=\"number\" min=\"1\" max=\"30\" class=\"form-control\" id=\"exampleInputPassword1\" placeholder=\"\"></div>"; */

 // If user checked 'Maqra'
 text += "<div class=\"form-check col-sm-1\" style=\"margin-top: 5px;\"><label class=\"form-check-label\" for=\"flexCheckDefault\"\"><input class=\"form-check-input\" type=\"checkbox\" value=\"\" id=\"maqra-checkbox\" onclick=\'displayMaqraAddData()\'>Maqra</label></div><div class=\"form-inline w-50\" id=\"maqra-choose-section\" style=\"display: none; text-align: center; margin: auto;\"><label for=\"exampleInputPassword1\" class=\"row align-items-center\" style=\"margin-right: 5px;\">From</label><input type=\"number\" min=\"1\" max=\"240\" class=\"form-control\" id=\"minmaqra\" placeholder=\"\"><label for=\"exampleInputPassword1\" style=\"margin: 0 5px 0 5px;\" class=\"row align-items-center\">To</label><input type=\"number\" min=\"1\" max=\"240\" class=\"form-control\" id=\"maxmaqra\" placeholder=\"\"></div>";

 // If user checked 'Page'
 text += "<div class=\"form-check col-sm-1\" style=\"margin-top: 5px;\"><label class=\"form-check-label\" for=\"flexCheckDefault\"\"><input class=\"form-check-input\" type=\"checkbox\" value=\"\" id=\"page-checkbox\" onclick=\'displayPageAddData()\'>Page</label></div><div class=\"form-inline w-50\" id=\"page-choose-section\" style=\"display: none; text-align: center; margin: auto;\"><label for=\"exampleInputPassword1\" class=\"row align-items-center\" style=\"margin-right: 5px;\">From</label><input type=\"number\" min=\"1\" max=\"604\" class=\"form-control\" id=\"minpage\" placeholder=\"\"><label for=\"exampleInputPassword1\" style=\"margin: 0 5px 0 5px;\" class=\"row align-items-center\">To</label><input type=\"number\" min=\"1\" max=\"604\" class=\"form-control\" id=\"maxpage\" placeholder=\"\"></div>";

 text += "<button type=\"button\" class=\"btn btn-secondary\" data-dismiss=\"modal\" style=\"margin-top: 10px;\">Close</button><button type=\"submit\" style=\"margin-top: 10px; margin-left: 5px;\" class=\"btn btn-primary\" onclick=\"addData()\">Add</button></form></div><div class=\"modal-footer\"></div></div></div></div>";

 counter = 2;
 for (let goals in goalsList) {
  // getting the current data and calculate the percentage
  var currentData = goalsList[goals];
  var juzIsExist = false;
  var maqraIsExist = false;
  ...
  for (let items in currentData) {
   if (currentData[items].includes("Juzu\'")) {
    juzIsExist = true;
    juzExist.push(currentData[items]);
   } else if (currentData[items].includes("Maqra\'")) {
    maqraIsExist = true;
    maqraExist.push(currentData[items]);
   } else if (currentData[items].includes("Page")) {
    pageIsExist = true;
    pageExist.push(currentData[items]);
   }
   totalItems++;
  }

  // getting da percentage
  var totalCompleted = 0;
  for (let items in juzExist) {
   for (let juz in juzList) {
    if (juzExist[items] == juz) {
     if (juzList[juz] == 1) {
      totalCompleted++;
     }
    }
  ...
  var percentage = ((totalCompleted / totalItems) * 100).toFixed(1);
  var isDone = false;
  if (percentage == 100) {
   isDone = true;
  }
  //========================
  var currentItemsModal = "<div class=\"modal fade\" id=\"" + goals.replace(/ /g, "-") + "\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"addgoals\" aria-hidden=\"true\"> <div class=\"modal-dialog\" role=\"document\"><div class=\"modal-content\"><div class=\"modal-header\"><h5 class=\"modal-title\" id=\"addgoals\">" + goals + "</h5><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button></div><div class=\"modal-body\">";

  // Display specified goals
  if (totalItems != 0) {
   currentItemsModal += "<p class=\"\" style=\"margin: 0; padding: 0;\">Goals :</p><ul class=\"list-group list-group-flush\">";
   if (juzIsExist) {
    if (juzExist.length == 1) {
     currentItemsModal += "<li class=\"list-group-item\">" + juzExist[0] + "</li>";
    } else {
     currentItemsModal += "<li class=\"list-group-item\">" + juzExist[0] + " - " + juzExist[juzExist.length - 1] + "</li>";
    }
   }
   if (maqraIsExist) {
    if (maqraExist.length == 1) {
     currentItemsModal += "<li class=\"list-group-item\">" + maqraExist[0] + "</li>";
    } else {
     currentItemsModal += "<li class=\"list-group-item\">" + maqraExist[0] + " - " + maqraExist[maqraExist.length - 1] + "</li>";
    }
   }
   if (pageIsExist) {
    if (pageExist.length == 1) {
     currentItemsModal += "<li class=\"list-group-item\">" + pageExist[0] + "</li>";
    } else {
     currentItemsModal += "<li class=\"list-group-item\">" + pageExist[0] + " - " + pageExist[pageExist.length - 1] + "</li>";
    }
   }
   currentItemsModal += "</ul><div style=\"height: 20px;\" class=\"progress\"><div class=\"progress-bar\" role=\"progressbar\" style=\"width: " + percentage + "%;\" aria-valuenow=\'" + percentage + "\' aria-valuemin=\"0\" aria-valuemax=\"100\">" + percentage + "%</div></div>";
  } else {
   currentItemsModal += "<p class=\"\">Goals is not specified.</p>"
  }

  //=========================
  currentItemsModal += "<form method=\"get\"><button type=\"button\" class=\"btn btn-secondary\" data-dismiss=\"modal\" style=\"margin-top: 10px;\">Close</button><button type=\"submit\" style=\"margin-top: 10px; margin-left: 5px;\" class=\"btn btn-danger\" onclick=\"removeData(\'" + goals.replace(/ /g, '-') + "\')\">Delete</button>";

  currentItemsModal += "</form></div><div class=\"modal-footer\"></div></div></div></div>";
  if (counter % 4 == 1) {
   text += "<div class=\"row\">";
  }
  if (counter % 4 != 0) {
   if (totalItems == 0) {
    text += "<div class=\"col\"><button type=\"button\" class=\"btn btn-warning goals\" data-toggle=\"modal\" data-target=\"#" + goals.replace(/ /g, "-") + "\">" + goals + "</button>" + currentItemsModal + "</div>";
   } else {
    if (isDone) {
     text += "<div class=\"col\"><button type=\"button\" class=\"btn btn-success goals\" data-toggle=\"modal\" data-target=\"#" + goals.replace(/ /g, "-") + "\">" + goals + "<h1 class=\"display-4 text-center\">" + percentage + "%</h1></button>" + currentItemsModal + "</div>";
    } else {
     text += "<div class=\"col\"><button type=\"button\" class=\"btn btn-light border border-primary text-primary goals\" data-toggle=\"modal\" data-target=\"#" + goals.replace(/ /g, "-") + "\">" + goals + "<h1 class=\"display-4 text-center\">" + percentage + "%</h1></button>" + currentItemsModal + "</div>";
    }
   }
  }
  else {
   if (totalItems == 0) {
    text += "<div class=\"col\"><button type=\"button\" class=\"btn btn-warning goals\" data-toggle=\"modal\" data-target=\"#" + goals.replace(/ /g, "-") + "\">" + goals + "</button>" + currentItemsModal + "</div></div>";
   } else {
    if (isDone) {
     text += "<div class=\"col\"><button type=\"button\" class=\"btn btn-success goals\" data-toggle=\"modal\" data-target=\"#" + goals.replace(/ /g, "-") + "\">" + goals + "<h1 class=\"display-4 text-center\">" + percentage + "%</h1></button>" + currentItemsModal + "</div></div>";
    } else {
     text += "<div class=\"col\"><button type=\"button\" class=\"btn btn-light border border-primary text-primary goals\" data-toggle=\"modal\" data-target=\"#" + goals.replace(/ /g, "-") + "\">" + goals + "<h1 class=\"display-4 text-center\">" + percentage + "%</h1></button>" + currentItemsModal + "</div></div>";
    }
   }
  }
  counter++;
 ...
 text += "</div>";
 document.getElementById("goals-list").innerHTML = text;
}
...
function loadData() {
 if (localStorage.getItem("goalsdata") != null) {
  ...
  localStorage.setItem("goalsdata", JSON.stringify(goalsList));
 }
 loadGoals();
}

function addData() {
 var juzCB = document.getElementById("juz-checkbox");
 var maqraCB = document.getElementById("maqra-checkbox");
 var pageCB = document.getElementById("page-checkbox");
 var newData = document.getElementById("new-goals").value;
 var choosenList = [];
 ...
 // check for juz data
 if (juzCB.checked == true) {
  var min = document.getElementById("minJuz").value;
  var max = document.getElementById("maxJuz").value;
  if (min == "" || max == "") {
   alert("Please fill all input!");
   return false;
  }
  if (parseInt(max) - parseInt(min) < 0) {
   alert("Invalid range for Juz!");
   return false;
  }
  let counterJuz = 1;
  for (let juz in juzList) {
   if (counterJuz >= min && counterJuz <= max) {
    choosenList.push(juz);
   }
   if (counterJuz > max) {
    break;
   }
 ...
 // check for maqra data
 if (maqraCB.checked == true) {
  var min = document.getElementById("minmaqra").value;
  var max = document.getElementById("maxmaqra").value;
  if (min == "" || max == "") {
   alert("Please fill all input!");
   return false;
  }
  if (parseInt(max) - parseInt(min) < 0) {
   alert("Invalid range for Maqra!");
   return false;
  }
  let counterMaqra = 1;
  for (let maqra in maqraList) {
   if (counterMaqra >= min && counterMaqra <= max) {
    choosenList.push(maqra);
   }
   if (counterMaqra > max) {
    break;
   }
 ...
 // check for page data
 if (pageCB.checked == true) {
  var min = document.getElementById("minpage").value;
  ...
  if (parseInt(max) - parseInt(min) < 0) {
   alert("Invalid range for Page!");
   return false;
  }
  let counterPage = 1;
  for (let page in pageList) {
   if (counterPage >= min && counterPage <= max) {
    choosenList.push(page);
   }
   if (counterPage > max) {
    break;
   }
 ...
 goalsList[newData] = choosenList;
 localStorage.removeItem("goalsdata");
 localStorage.setItem("goalsdata", JSON.stringify(goalsList));
}

function removeData(selectedData) {
 var oriData = selectedData.replace(/-/g, " ");
 delete goalsList[oriData];
 localStorage.removeItem("goalsdata");
 localStorage.setItem("goalsdata", JSON.stringify(goalsList));
}

function displayJuzAddData() {
 var juzChoosen = document.getElementById("juz-choose-section");
 var juzCB = document.getElementById("juz-checkbox");
 if (juzCB.checked == true) {
  juzChoosen.style.display = "flex";
 } else {
  juzChoosen.style.display = "none";
 }
}

function displaySurahAddData() {
 var juzChoosen = document.getElementById("surah-choose-section");
 var surahCB = document.getElementById("surah-checkbox");
 if (surahCB.checked == true) {
  juzChoosen.style.display = "flex";
 } else {
  juzChoosen.style.display = "none";
 }
...
function displayMaqraAddData() {
 var maqraChoosen = document.getElementById("maqra-choose-section");
 var maqraCB = document.getElementById("maqra-checkbox");
 if (maqraCB.checked == true) {
  maqraChoosen.style.display = "flex";
 } else {
  maqraChoosen.style.display = "none";
 }
}

function displayPageAddData() {
 var pageChoosen = document.getElementById("page-choose-section");
 var pageCB = document.getElementById("page-checkbox");
 if (pageCB.checked == true) {
  pageChoosen.style.display = "flex";
 } else {
  pageChoosen.style.display = "none";
 }
}
